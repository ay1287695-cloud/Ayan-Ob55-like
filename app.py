# AYAN LIKE API SRC UID PASSWORD 
# POWERED BY : AYAN
# DEVELOPER : AYAN
...
# এই রাউটটি ক্রোমে লিংক দিলে গাইডলাইন দেখাবে
...
def home():
    ...
    return jsonify({
        "status": "Online",
        "developer": "Ayan",
        "message": "Welcome to Ayan Free Fire Like API!",
        "instructions": {
            "format": f"{base_url}/like?uid=YOUR_UID&server_name=SERVER&key=ayan",
            "required_parameters": {
                "uid": "Player Target UID (e.g. 123456789)",
                "server_name": "BD, IND, BR, US, SAC, NA, RU",
                "key": "ayan"
            },
            "example_url": f"{base_url}/like?uid=123456789&server_name=BD&key=ayan"
        }
    })
...
def handle_requests():
    ...
    if key != "ayan":
        return jsonify({"error": "Invalid or missing API key 🔑. Powered by Ayan"}), 403
...
def reset_cache():
    ...
    global liked_cache
    liked_cache.clear()
    return jsonify({"message": "Cache cleared", "credit": "Ayan"})

if __name__ == '__main__':
    print("🚀 Server started - Smart Like System!")
    print("📁 Powered by AYAN")
